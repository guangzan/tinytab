...
function handleExportSettings() {
	...
	const { enginesData, theme } = store.state
...
<template>
	<pannel title="导入与导出" desc="为防止配置丢失，请务必妥善保存配置文件">
		<n-space>
			<n-button @click="handleImportSettings">
				<template #icon>
					<n-icon>
						<download />
					</n-icon>
			...
			<n-button type="primary" @click="handleExportSettings">
				<template #icon>
					<n-icon>
						<arrow-up />
					</n-icon>
	...
	>
		<div class="flex flex-col justify-center items-center my-8 mx-auto">
			...
			<div class="mt-8">{{ downloading ? '文件生成中' : '文件生成成功' }}</div>
		</div>
	</n-modal>
</template>
